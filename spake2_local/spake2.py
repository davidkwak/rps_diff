...
def finalize_SPAKE2(idA, idB, X_msg, Y_msg, K_bytes, pw):
    # This  transcript is not following SPAKE2 standard
    # https://tools.ietf.org/id/draft-irtf-cfrg-spake2-10.html#rfc.authors
    ...
    # print(f"sha256(pw).digest() : {sha256(pw).digest()}")
    # print(f"sha256(idA).digest() : {sha256(idA).digest()}")
    # print(f"sha256(idB).digest() : {sha256(idB).digest()}")    
...
class _SPAKE2_Base:
    ...
    def __init__(self, password,
        ...
        assert isinstance(password, bytes)  # a. 문자열 비밀번호를 Byte로 형변환
        self.pw = password
    ...
    def start(self):
        ...
        g = self.params.group
        self.xy_scalar = g.random_scalar(self.entropy_f) # 2. 임의의 비밀 키 생성
        self.xy_elem = g.Base.scalarmult(self.xy_scalar) # 3. 공개키 생성 a. 2에서 생성한 비밀 키(self.xy_scalar)와 타원 곡선상의 생성자 G(g.Base)를 스칼라 곱셈(scalarmult) 연산
        self.compute_outbound_message() #4. PAKE 메시지 생성
    ...
    def compute_outbound_message(self):
        pw_blinding = self.my_blinding().scalarmult(self.pw_scalar) #a. 1에서 생성한 공동 비밀번호 scalar(self.pw_scalar)와 지정된 M(self.my_blinding())을 스칼라 곱셈(scalarmult)
        message_elem = self.xy_elem.add(pw_blinding) # b. a의 결과에 3에서 생성한 공개키를 스칼라 덧셈
    ...
    def finish(self, inbound_side_and_message):
        ...
        key = sha256(K_bytes).digest() #xyG 값 
...
class _SPAKE2_Asymmetric(_SPAKE2_Base):
    ...
    def _extract_message(self, inbound_side_and_message):
        other_side = inbound_side_and_message[0:1] #Flag 
        inbound_message = inbound_side_and_message[1:] #SPAKE Message 

        if other_side not in (SideA, SideB):
            raise OffSides("I don't know what side they're on")
        if self.side == other_side:
            if self.side == SideA:
                raise OffSides("I'm Phone, but I got a message from Phone (not Car).")
            else:
                raise OffSides("I'm Car , but I got a message from Car (not Phone).")
    ...
    def _serialize_to_dict(self):
        ...
        print(f"_serialize_to_dict d : {d}")
        return d
...
class SPAKE2_A(_SPAKE2_Asymmetric):
    side = SideA
    def my_blinding(self): 
        # print(f"M:{self.params.M.to_bytes()}")                  
        return self.params.M
    def my_unblinding(self): 
        # print(f"N:{self.params.N.to_bytes()}")                          
        return self.params.N
    def X_msg(self): return self.outbound_message
    def Y_msg(self): return self.inbound_message
    def key_confirmation(self,TT,confrimation_key):
        hashed_TT = sha256(TT.encode()).hexdigest()
        Ka = hashed_TT[:int(len(hashed_TT)/2)]     

        ht = Hkdf(salt=bytes.fromhex(''),
        input_key_material=bytes.fromhex(b"ConfirmationKeys".hex()))
        
        okm = ht.expand(info=bytes.fromhex(Ka))
        Kc = sha256(okm).hexdigest()
        KcB = Kc[int(len(Kc)/2):]        
        ConfirmationKey_B = hmac.new(KcB.encode(),TT.encode(),sha256).hexdigest()

        # Generate opponent's  KcB 
        if confrimation_key == ConfirmationKey_B:
            return True 
        else:
            return False
...
class SPAKE2_B(_SPAKE2_Asymmetric):
    ...
    def key_confirmation(self,TT,confrimation_key):
        ...
        Kc = sha256(okm).hexdigest()
        KcA = Kc[:int(len(Kc)/2)]        
        ConfirmationKey_A = hmac.new(KcA.encode(),TT.encode(),sha256).hexdigest()

        # Generate opponent's  KcB 
        if confrimation_key == ConfirmationKey_A:
            return True 
        else:
            return False 
